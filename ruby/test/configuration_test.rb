...
require_relative '../lib/github_wiki_organiser/configuration'
require_relative '../lib/github_wiki_organiser/home'
require_relative '../lib/github_wiki_organiser/unknown_wiki_count_list_exporter'
...
class ConfigurationTest < ApplicationTest
  ...
  def test_default_group_bys_and_languages
    configuration = GithubWikiOrganiser::Configuration.new(base_path:)
  ...
  def test_urls_derived_from_organisation_and_repository
    configuration = GithubWikiOrganiser::Configuration.new(base_path:, organisation: 'test-org',
  ...
  def test_empty_values_are_treated_as_absent
    configuration = GithubWikiOrganiser::Configuration.new(base_path:, organisation: '', repository: '')
  ...
  def test_explicit_options_win_over_the_config_file
    write_config_file
    configuration = GithubWikiOrganiser::Configuration.new(base_path:, organisation: 'cli-org')
  ...
  def test_config_file_language_passes_validation
    ...
    application = GithubWikiOrganiser::Application.new(base_path:, group_by: 'Owner', language: 'French',
                                                       home_overflow: 'false')
  ...
  def test_exporter_honours_custom_output_filename
    _, path_to_export = GithubWikiOrganiser::UnknownWikiCountListExporter.run(base_path:, group_by:, language:,
  ...
  def write_config_file
    File.write(File.join(base_path, GithubWikiOrganiser::Configuration::CONFIG_FILENAME), CUSTOM_CONFIG)
  end
end
