...
- Ruby gem packaging: `Spreen::Wiki` module under `ruby/lib/`, `spreen-wiki.gemspec`, `exe/spreen`, RBS signatures, and the four default Home templates shipped as gem data.
- Python packaging: `spreen_wiki` package under `python/src/`, full PyPI metadata in `pyproject.toml`, `spreen` console script, `py.typed`, and the four default Home templates shipped as package data.
